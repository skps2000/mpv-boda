...
English: [README.md](README.md) · UI 언어는 기본 영어입니다. 한국어로 쓰려면
`script-opts/boda.conf` 에서 `language=ko` 로 바꾸세요.
...
- **알아볼 수 있는 이름** — 파일 안에 박힌 제목이 아니라 파일 이름으로 표시해서, 보다 보면 목록 이름이 바뀌는 일이
  없습니다. 재생목록 파일(m3u)을 열면 거기 적힌 이름을 그대로 씁니다
- **대기 화면 이어보기** — 재생이 끝나거나 Ctrl+F4로 멈추면 보던 폴더가 먼저, 그 아래에 최근 본 영상이 진행률과 함께
  뜹니다. 숫자키로 바로 열고, 각 목록은 화면에서 바로 지울 수 있습니다
- **아래쪽 탐색바** — 클릭 탐색, 챕터 표시, A-B 구간 표시, 볼륨, 마우스 올리면 미리보기(ffmpeg 필요)
- **메뉴** (우클릭 또는 F4) — 재생·열기·목록·즐겨찾기·영상·소리·자막·색감·창·캡처·복사를 한 곳에서.
  현재 상태가 체크로 보이고, 목록 위에서 누르면 그 항목 메뉴(탐색기에서 보기 포함)가 뜹니다
...
|---|---|
| `Space` | 재생 / 일시정지 |
| `←` `→` | 8초 이동 (Ctrl 30초, Shift 1분 40초) — 네 간격 모두 설정 가능 |
| `↑` `↓` / 휠 | 음량 |
...
| `seek_arrow` | `8` | `←` `→` 이동 초 |
| `seek_ctrl` | `30` | `Ctrl+←` `Ctrl+→` 이동 초 |
| `seek_shift` | `100` | `Shift+←` `Shift+→` 이동 초 |
| `seek_alt` | `300` | `Ctrl+Alt+←` `Ctrl+Alt+→` 이동 초 |
